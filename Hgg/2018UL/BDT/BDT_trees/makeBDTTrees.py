...
def create_BDT_trees():
    ...
    for sampleName, sample in samples.items():
        ...
        print("Number of files:", len(sample['name']))
        print("=" * 80)
        ...
        for inputFile in sample['name']:
            ...
            if tree is None:
                print("Error: Tree", treeName, "not found.")
                fin.Close()
                continue
            ...
            for branch in branches_to_enable:
                tree.SetBranchStatus(branch, 1)  # Enable required branches
            ...
            outTree = ROOT.TTree("Events", "BDT Tree")
            ...
            print(f"Selected events : {nSelected}/{nEntries}")

        print("\nFinished processing sample:", sampleName)
